...
*/
        ...
        public
    {
        ...
        for (uint256 i = 0; i < abs.length; i++) {
            ...
            _userGet[i] = _ABamount[i].mul(calForceOf[msg.sender]).div(totalCalForce);


            _ab.mintToken(msg.sender, _userGet[i].mul(_times));          
        }
...
contract DropAlgorithms {
    function expected() external pure returns(uint256);
}